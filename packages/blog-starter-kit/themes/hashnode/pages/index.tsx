...
import PublicationTagCloud, { PublicationTagCloudItem } from '../components/publication-tag-cloud';
import { resizeImage } from '../utils/image';

const REVALIDATION_INTERVAL_POST_VIEWS_ACTIVE = 60 * 60; // 1 hour
const REVALIDATION_INTERVAL = 60 * 60 * 24 * 30; // 1 month
const TAG_CLOUD_POST_LIMIT = 100;

const HomePageTagCloudQuery = /* GraphQL */ `
	query HomePageTagCloud($host: String!, $first: Int!) {
...
type HomePageTagCloudQueryResponse = {
	publication?: {
		posts: {
			edges: Array<{
				node: {
...
function getTagCloudItems(data?: HomePageTagCloudQueryResponse): Array<PublicationTagCloudItem> {
	const tagsBySlug = new Map<string, PublicationTagCloudItem>();

	data?.publication?.posts.edges.forEach((edge) => {
		edge.node.tags?.forEach((tag) => {
			const existingTag = tagsBySlug.get(tag.slug);
...
export default function Index(
	...
	const { host, publication, initialLimit, tagCloudItems } = props;
	...
	const [{ data }] = useQuery({
		...
		variables: { host, first: initialLimit },
	});

	const { posts } = data?.publication!;

	const fetchedOnce = posts.edges.length > initialLimit;
	const postEdges = pinnedPost
		? posts.edges.filter((edge) => edge.node.id !== pinnedPost.id)
		: posts.edges;
		...
		<AppProvider publication={publication}>
			<Layout>
				...
				<div>
					...
					<PublicationTagCloud tags={tagCloudItems} />
...
export const getStaticProps = async () => {
	...
	let tagCloudItems: Array<PublicationTagCloudItem> = [];

	try {
		const tagCloudResponse = await request<HomePageTagCloudQueryResponse>(
			HASHNODE_GQL_ENDPOINT,
			HomePageTagCloudQuery,
			{
				host,
				first: TAG_CLOUD_POST_LIMIT,
			},
			createHeaders({ byPassCache: false }),
		);
		tagCloudItems = getTagCloudItems(tagCloudResponse);
	} catch (error) {
		console.error('Error while fetching home page tag cloud', {
			error,
			host,
		});
	}
